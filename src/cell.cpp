...
cell::set_init_data(const spec_v& init_s, const cell_input& init_data)
{
  ...
  //cell_st.g_change.resize(numSpec, 0.0);
  ...
  //cell_st.sizeBins.assign(init_data.inp_size_dist.begin(),init_data.inp_size_dist.end());
  cell_st.vd.assign(init_data.inp_vd.begin(),init_data.inp_vd.end());
  // need to check this. I may need another array for nuc vs nuc +dest vs dest
  ...
  cell_st.erosion_dadt.resize(cell_st.numReact*cell_st.numBins);
}
...
void cell::calc_state_var_shock(const double time)
{
  bool shock_present = env_shock_bool_interp(time);
  if(shock_present)
  {
    double shock_velo = env_shock_velo_interp(time);
    std::fill(cell_st.vd.begin(),cell_st.vd.end(),shock_velo);
  }
}
...
void cell::nucleate(const std::vector<double>& x)
{
  ...
  for (size_t gidx = 0; gidx < cell_st.numReact; ++gidx) 
  {
    ...
    // std::string elem_sym = net->species[cell_st.parts[i].ks_idx];
    // auto at_mass = elm.elements.at(elem_sym).mass;
    // cell_st.parts[i].ks_react_mass = at_mass * amu2g;
    ...
    cell_st.parts[gidx].catS = (stdP / cell_st.kT) * exp(-delg_reduced);
